...
func (suite *AnteTestSuite) TestEnsureMempoolFees() {
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		...
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(0)),
	)
...
func (suite *AnteTestSuite) TestMultipleGlobalMinimumFees() {
	...
	// keys and addresses
	priv1, _, addr1 := testdata.KeyTestPubAddr()
	coins := sdk.NewCoins(sdk.NewCoin("atom", sdk.NewInt(3000000000)), sdk.NewCoin("ukii", sdk.NewInt(3000000000)))
	err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, addr1, coins)
	suite.Require().NoError(err)
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("atom", sdk.NewDec(100)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(100)), sdk.NewDecCoinFromDec("ukii", sdk.NewDec(1))})
	// 750000 < 15000 * 100
	tx, _ := suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("atom", sdk.NewDec(100)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(200)), sdk.NewDecCoinFromDec("ukii", sdk.NewDec(1))})
	// 750000 < 15000 * 200
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)), sdk.NewDecCoinFromDec("ukii", sdk.NewDec(1))})
	// 750000 = 15000 * 50
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored on equal fee for global gasPrice")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)))
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{sdk.NewDecCoinFromDec("atom", sdk.NewDec(1)), sdk.NewDecCoinFromDec("ukii", sdk.NewDec(5))})
	// 750000 = 15000 * 50
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored on equal fee for global gasPrice")
	...
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		...
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(2)),
	)
	suite.app.ParamsKeeper.SetFeesParams(suite.ctx, feeParam)
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(2)), // equal to global minimum
	})
	tx, _ = suite.createTestTxWithGas(msg, 750000, 15000, priv1, "atom")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored, fee is sufficient in 'ukii' denomination")
	...
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		...
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(10)),  // greater than global minimum
	})
	...
	// Test case: fee provided in 'ukii' denomination is greater than the global minimum gas prices
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(2)),
	...
	tx, _ = suite.createTestTxWithGas(msg, 1500000, 15000, priv1, "ukii")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Require().Nil(err, "Decorator should not have errored, fee is sufficient in 'ukii' denomination")

	// Test case: fee provided in 'ukii' denomination is less than the global minimum gas prices
	feeParam = suite.app.ParamsKeeper.GetFeesParams(suite.ctx)
	feeParam.GlobalMinimumGasPrices = sdk.NewDecCoins(
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(10)),
	...
	suite.ctx = suite.ctx.WithMinGasPrices([]sdk.DecCoin{
		sdk.NewDecCoinFromDec("atom", sdk.NewDec(50)),
		sdk.NewDecCoinFromDec("ukii", sdk.NewDec(5)), // less than global minimum
	})
	tx, _ = suite.createTestTxWithGas(msg, 100000, 15000, priv1, "ukii")
	_, err = antehandler(suite.ctx, tx, false)
	suite.Assert().ErrorContains(err, "insufficient fees")
}
