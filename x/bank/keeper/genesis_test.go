...
func (suite *IntegrationTestSuite) TestExportGenesis() {
	...
	expectedWeiBalances := []types.WeiBalance{
		{Amount: keeper.OneUkiiInWei.Sub(sdk.OneInt()), Address: expectedBalances[0].Address},
...
func (suite *IntegrationTestSuite) TestTotalSupply() {
	...
	weiBalances := []types.WeiBalance{
		...
		{Amount: keeper.OneUkiiInWei.Sub(sdk.OneInt()), Address: "cosmos1m3h30wlvsf8llruxtpukdvsy0km2kum8g38c8q"},
	}
	...
	testcases := []struct {
	...
	}{
		{
			...
			nil, true, "genesis supply is incorrect, expected 1wrongcoin, got 21barcoin,11foocoin,1ukii",
		},
		{
