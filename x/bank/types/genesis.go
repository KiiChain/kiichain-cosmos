...
func getTotalSupply(genState *GenesisState) (sdk.Coins, error) {
	...
	weiInUkii, weiRemainder := SplitUkiiWeiAmount(totalWeiBalance)
	...
	if err != nil {
		if !weiInUkii.IsZero() {
			return nil, fmt.Errorf("base denom is not registered %s yet there exists wei balance %s", err, weiInUkii)
		}
	} else {
		totalSupply = totalSupply.Add(sdk.NewCoin(baseDenom, weiInUkii))
	}
